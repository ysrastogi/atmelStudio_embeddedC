...
void adc_init(){
	...
	ADCSRA_reg	= ((1 << ADEN_bit) | (1 << ADPS2_bit) | (1 << ADPS1_bit)  | (1 << ADIE_bit)) | (ADCSRA_reg  & (~((1 << ADATE_bit) | (1 << ADIF_bit) | (1 << ADPS0_bit) | (1 << ADSC_bit))));
...
void select_adc_channel( unsigned char channel_num ){
	...
	if (channel_num == left_wl_sensor_channel)
	{
		ADMUX_reg = (1 << MUX0_bit) | ( ADMUX_reg & (~((1 << MUX1_bit) | (1 << MUX2_bit) | (1 << MUX3_bit) | (1 << MUX4_bit)))); //set the MUX[5:0] bits to ADC channel number 3~~1 done
		ADCSRB_reg &= ~(1 << MUX5_bit);
	}
	if (channel_num == center_wl_sensor_channel)
	{
		ADMUX_reg &= ~((1 << MUX0_bit) | (1 << MUX1_bit) | (1 << MUX2_bit) | (1 << MUX3_bit) | (1 << MUX4_bit));
		ADCSRB_reg &= ~((1 << MUX5_bit));//set the MUX[5:0] bits to ADC channel number 0 done
	}
	if (channel_num == sharp_sensor_channel)
	{
		ADMUX_reg &= ~((1 << MUX0_bit) | (1 << MUX1_bit) | (1 << MUX2_bit) | (1 << MUX3_bit) | (1 << MUX4_bit));
		ADCSRB_reg &= ~((1 << MUX5_bit)); //set the MUX[5:0] bits to ADC channel number 2~~0 done
	}
	if (channel_num == right_wl_sensor_channel)
	{
		ADMUX_reg = (1 << MUX1_bit) | (ADMUX_reg & (~((1 << MUX0_bit) | (1 << MUX2_bit) | (1 << MUX3_bit) | (1 << MUX4_bit)))); //set the MUX[5:0] bits to ADC channel number 1~~2 done
		ADCSRB_reg &= ~(1 << MUX5_bit);
	}
	if (channel_num == ir_prox_3_sensor_channel)
	{
		ADMUX_reg = ((1 << MUX0_bit) | (1 << MUX1_bit)) | (ADMUX_reg & (~((1 << MUX2_bit) | (1 << MUX3_bit) | (1 << MUX4_bit)))); //set the MUX[5:0] bits to ADC channel number 6~~3 done
		ADCSRB_reg &= ~(1 << MUX5_bit);
	}
	if (channel_num == ir_prox_4_sensor_channel)
	{
		ADMUX_reg = (1 << MUX2_bit) | (ADMUX_reg & (~((1 << MUX0_bit) | (1 << MUX1_bit) | (1 << MUX3_bit) | (1 << MUX4_bit)))); //set the MUX[5:0] bits to ADC channel number 7~~4 done
		ADCSRB_reg &= ~(1 << MUX5_bit);
	}
	if (channel_num == ir_prox_5_sensor_channel)
	{
		ADMUX_reg |= ((1 << MUX2_bit) | (1 << MUX0_bit)) | (ADMUX_reg & (~((1 << MUX1_bit) | (1 << MUX3_bit) | (1 << MUX4_bit)))); //set the MUX[5:0] bits to ADC channel number 8~~5 done
		ADCSRB_reg &= ~(1 << MUX5_bit);
	}
